...
export class ProductOrderService {
  public resourceUrl = this.applicationConfigService.getEndpointFor('api/product-orders');
  ...
  protected convertDateFromClient(productOrder: IProductOrder): IProductOrder {
    return Object.assign({}, productOrder, {
      placeDate: productOrder.placeDate?.isValid() ? productOrder.placeDate.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.placeDate = res.body.placeDate ? dayjs(res.body.placeDate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((productOrder: IProductOrder) => {
        productOrder.placeDate = productOrder.placeDate ? dayjs(productOrder.placeDate) : undefined;
      });
    }
    return res;
  }
